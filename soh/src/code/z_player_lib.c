...
s32 isPlayerInHorizontalAttack(PlayState* play) {
    Player* player = GET_PLAYER(play);
    return isPlayerInHorizontalSlash(play) || (player->swordAnimation == PLAYER_MWA_HAMMER_SIDE && player->swordState);
}

s32 isPlayerInVerticalAttack(PlayState* play) {
    Player* player = GET_PLAYER(play);
    return isPlayerInVerticalSlash(play) || (player->swordAnimation == PLAYER_MWA_HAMMER_FORWARD && player->swordState);
}
