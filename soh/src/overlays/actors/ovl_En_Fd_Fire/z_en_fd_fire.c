...
void EnFdFire_WaitToDie(EnFdFire* this, GlobalContext* globalCtx) {
    if (this->deathTimer == 280) {
        ...
        if ((valType % 12) == 0) {
            Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_FIREFLY, this->actor.world.pos.x, this->actor.world.pos.y+30,
                                       this->actor.world.pos.z, 0, this->actor.world.rot.y, 0, 0x0);
            this->actionFunc = EnFdFire_Disappear;
        }
        else if ((valType % 4) == 0) {
            Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_BW, this->actor.world.pos.x, this->actor.world.pos.y,
...
void EnFdFire_DanceTowardsPlayer(EnFdFire* this, GlobalContext* globalCtx) {
    ...
    idx = ((globalCtx->state.frames / 10) + (this->actor.params & 0xF)) % ARRAY_COUNT(angles);
